(function(){
    ...
    function tweenCircle(c, dir) {
		...
		var xPos = c.x + -100+Math.random()*200;
		var yPos = c.y + -100+Math.random()*200;
		if (xPos < 0) xPos = -xPos;		
		if (yPos < 0) yPos = -yPos;
            ...
            if(c.movement == 'float') {
                // c.tween = TweenLite.to(c, 5 + Math.random()*3.5, {x: c.x + -100+Math.random()*200, y: c.y + -100+Math.random()*200, ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.5,
                    // onComplete: function() {
                        // tweenCircle(c);
                    // }});
				c.tween = TweenLite.to(c, 5 + Math.random()*3.5, {x: xPos, y: yPos, ease:Quad.easeInOut, alpha: 0.2 + Math.random()*0.5,
                    onComplete: function() {
                        tweenCircle(c);
                    }});
            } else {
	...
	function addListeners() {
        ...
        btn.addEventListener('click', function(e) {
            ...
            for (var i=0; i<circles.length; i++) {
				if (circles[i].x  >= 0 && circles[i].x <= width && circles[i].y  >= 0 && circles[i].y <= height) count++;				
			}
